...
router.post('/signup', function (req, res) {
  ...
  try {
    ...
    return res.status(200).json({
      message: 'Кооистувач успішно зареєстрований',
      session,
    })
  } catch (err) {
    return res.status(400).json({
...
router.post('/recovery', function (req, res) {
  ...
  try{
    ...
    return res.status(200).json({
      message: 'Код для відновлення паролю відправлено на email',
    })
...
router.post('/recovery-confirm', function (req, res) {
  ...
  try {
    ...
    if(!user){
      return res.status(400).json({
        message: 'Користувач з таким email не дійсний',
      })
    }
